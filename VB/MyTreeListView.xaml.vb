...
Namespace DXTreeListSample
	...
	Public Class MyTreeListDataProvider
		...
		Public Sub New(ByVal view As TreeListView)
			MyBase.New(view)
		End Sub
		Protected Overrides Sub CalcSummary(ByVal summary As ISummaryItemOwner)
		...
		Private Sub UpdateGroupFooterSummaryContent(ByVal groupRowHandle As Integer)
			...
			For Each value In summaryItems
				...
				If (Not String.IsNullOrEmpty(summaryValues(col))) Then
                    summaryValues(Grid.Columns(item.FieldName)) += vbLf
				End If
                summaryValues(Grid.Columns(item.FieldName)) += item.SummaryType.ToString() & " = " & value.Value.ToString()
			Next value
			Tag = summaryValues
		End Sub
